...
export default function ForumScreen() {
  ...
  async function loadPosts({ showLoader = false } = {}) {
    try {
      setError("");
      if (showLoader) {
        setRefreshing(true);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <AppHeader
        title="Foro"
        subtitle="Comparte dudas y experiencias con otros usuarios."
        showBack
        onBack={() => router.replace("/home")}
      />

      <View style={styles.actions}>
        <PrimaryButton title="Crear publicación" onPress={() => router.push("/create-post")} />
        <PrimaryButton
          title="Actualizar"
          onPress={() => loadPosts({ showLoader: true })}
          loading={refreshing}
        />
      </View>
